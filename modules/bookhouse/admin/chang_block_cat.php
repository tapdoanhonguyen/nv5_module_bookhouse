...
if ($mod == 'weight' and $new_vid > 0) {
    ...
    $db->query($sql);
    $content = 'OK_' . $bid;
} elseif ($mod == 'useradd' and $bid > 0) {
    $new_vid = (intval($new_vid) == 1) ? 1 : 0;
    $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_block_cat SET useradd=' . $new_vid . ' WHERE bid=' . $bid;
    $db->query($sql);
    $content = 'OK_' . $bid;
    ...
    $db->query($sql);
    $content = 'OK_' . $bid;
} elseif ($mod == 'prior' and $bid > 0) {
    $count = $db->exec('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_block_cat SET prior=' . intval($new_vid) . ' WHERE bid=' . $bid);
    if ($count) {
        // Cập nhật ưu tiên bất động sản
        $result = $db->query('SELECT id FROM ' . NV_PREFIXLANG . '_' . $module_data . '_block WHERE bid=' . $bid);
        while (list ($id) = $result->fetch(3)) {
            $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET prior = ' . $new_vid . ' WHERE id=' . $id);
        }
    }
    $content = 'OK_' . $bid;
} elseif ($mod == 'groups' and $bid > 0) {
    $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_block_cat SET groups=' . $new_vid . ' WHERE bid=' . $bid;
    $db->query($sql);
    $content = 'OK_' . $bid;
}
