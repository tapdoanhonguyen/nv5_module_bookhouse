...
if ($nv_Request->isset_request('add', 'get') or $nv_Request->isset_request('edit', 'get')) {
    ...
    if (!$id) {
        ...
        $data['numprice'] = 0;
        $data['price_time'] = 0;
        $data['price_rent_time'] = 0;
        $data['money_unit'] = '';
        $data['money_rent_unit'] = '';
        $data['homeimgfile'] = '';
        $data['front'] = 0;
        $data['road'] = 0;
        $data['structure'] = '';
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        $data['product_status'] = $nv_Request->get_int('product_status', 'post', 1);
		...
		$data['price_rent'] = $nv_Request->get_string('price_rent', 'post', '');
        $data['price_rent'] = preg_replace('/[^0-9]/', '', $data['price_rent']);
        $data['numprice'] = $nv_Request->get_int('numprice', 'post', 0);
        $data['price_time'] = $nv_Request->get_int('price_time', 'post', 0);
        $data['price_rent_time'] = $nv_Request->get_int('price_rent_time', 'post', 0);
        $data['money_unit'] = $nv_Request->get_string('money_unit', 'post', 'vnd');
        $data['money_rent_unit'] = $nv_Request->get_string('money_rent_unit', 'post', 'vnd');
        ...
        } elseif (empty($data['catid'])) {$data['catid']=0;
        ...
        if (empty($error)) {
			...
			$data['orther_picture'] = implode('|', $array_otherimage);
            ...
            if ($id > 0) {
                ...
                $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET
				    ...
				    price_rent_time = :price_rent_time,
					money_unit = :money_unit,
					money_rent_unit = :money_rent_unit,
					typeid = :typeid,
					projectid = :projectid,
                ...
                $stmt->bindParam(':orther_picture', $data['orther_picture'], PDO::PARAM_STR);
                ...
                $stmt->bindParam(':price_rent', $data['price_rent'], PDO::PARAM_STR);
                $stmt->bindParam(':numprice', $data['numprice'], PDO::PARAM_INT);
                $stmt->bindParam(':price_time', $data['price_time'], PDO::PARAM_INT);
                $stmt->bindParam(':price_rent_time', $data['price_rent_time'], PDO::PARAM_INT);
                $stmt->bindParam(':money_unit', $data['money_unit'], PDO::PARAM_STR);
                $stmt->bindParam(':money_rent_unit', $data['money_rent_unit'], PDO::PARAM_STR);
                $stmt->bindParam(':way_id', $data['way_id'], PDO::PARAM_INT);
                $stmt->bindParam(':typeid', $data['typeid'], PDO::PARAM_INT);
                $stmt->bindParam(':product_status', $data['product_status'], PDO::PARAM_INT);
			...
			{
                $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (title, alias, catid, phong_khach, phong_ngu, phong_tam, hometext, bodytext, admin_id, addtime, edittime, exptime, code, area, size_v, size_h, price, price_rent, numprice, price_time, money_unit, price_rent_time, money_rent_unit, typeid, projectid, way_id, legal_id, homeimgfile, homeimgthumb, 
                homeimgalt, orther_picture, front, road, furniture, formid, tien_ich, structure, type, provinceid, districtid, wardid, address, maps, inhome, allowed_comm, hitstotal, showprice, contact_fullname, contact_email, contact_phone, contact_address, ordertime, product_status)
				...
				:code, :area, :size_v, :size_h, :price, :price_rent, :numprice, :price_time, :money_unit, :price_rent_time, :money_rent_unit, :typeid, :projectid, :way_id, :legal_id, :homeimgfile, ' . $data['homeimgthumb'] . ', :homeimgalt, :orther_picture, :front, :road, :furniture, :formid, :tien_ich, :structure, :type, :provinceid, :districtid, :wardid, :address, :maps, :inhome,
				:allowed_comm, 0, :showprice, :contact_fullname, :contact_email, :contact_phone, :contact_address, :ordertime, :product_status)';
                ...
                $data_insert['price_rent'] = $data['price_rent'];
                $data_insert['numprice'] = $data['numprice'];
                $data_insert['price_time'] = $data['price_time'];
                $data_insert['price_rent_time'] = $data['price_rent_time'];
                $data_insert['money_unit'] = $data['money_unit'];
                $data_insert['money_rent_unit'] = $data['money_rent_unit'];
                $data_insert['typeid'] = $data['typeid'];
                $data_insert['projectid'] = $data['projectid'];
                $data_insert['product_status'] = $data['product_status'];
	...
	if (!empty($data['orther_picture'])) {
		$otherimage = explode('|', $data['orther_picture']);
	} else {
		$otherimage = [];
	}
    ...
    foreach ($array_money_unit as $key => $value) {
	 ...
	 foreach ($array_money_unit as $key => $value) {
        $sl = $key == $data['money_rent_unit'] ? 'selected="selected"' : '';
        $xtpl->assign('UNIT', array(
            'key' => $key,
            'value' => $value,
            'selected' => $sl
        ));
        $xtpl->parse('main.money_rent_unit');
    }
    if (!empty($error)) {
    ...
    foreach ($array_price_time as $index => $value) {
	 ...
	 foreach ($array_price_time as $index => $value) {
        $sl = $index == $data['price_rent_time'] ? 'selected="selected"' : '';
        $xtpl->assign('PRICETIME', array(
            'index' => $index,
            'value' => $value['title'],
            'selected' => $sl
        ));
        $xtpl->parse('main.price_rent_time');
    }
    ...
    $db->select('id, title, alias, catid, price, provinceid, districtid, wardid, money_unit, status, is_queue, status_admin, addtime, numprice ')
        ->order('addtime DESC')
        ->limit($per_page)
